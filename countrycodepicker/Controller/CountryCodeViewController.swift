...
//
//  Created by Chetan Singh Negi on 03/08/23.
...
class CountryCodeViewController: UIViewController {
   ...
   weak var delegate: CountryCodeViewControllerDelegate?
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "SelectCountry"
        self.countryCodeTableView.separatorStyle = .none
...
extension CountryCodeViewController : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
           ...
           let countryCode = list[indexPath.row].extensionCode ?? ""
           let flag = list[indexPath.row].flag ?? ""
           cell?.textLabel?.text = "\(countryCode) \(flag) \(list[indexPath.row].name ?? "")"
           return cell!
       }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let selectedCountryCodeExtension = list[indexPath.row].extensionCode{
            delegate?.didSelectedCountryCode(countryCode: selectedCountryCodeExtension)
            print(indexPath.row)
...
extension String {
    static func flag(for code: String) -> String? {
        func isLowercaseASCIIScaler(_ scalar: Unicode.Scalar) -> Bool {
            return scalar.value >= 0x61 && scalar.value <= 0x7A
        }
        
        func regionalIndicatorSymbol(for scalar: Unicode.Scalar) -> Unicode.Scalar {
            precondition(isLowercaseASCIIScaler(scalar))
            return Unicode.Scalar(scalar.value + (0x1F1E6 - 0x61))!
        }
        
        let lowercasedCode = code.lowercased()
        guard lowercasedCode.count == 2 else { return nil }
        guard lowercasedCode.unicodeScalars.reduce(true, { accum, scalar in accum && isLowercaseASCIIScaler(scalar) }) else { return nil }
